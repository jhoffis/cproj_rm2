...
image_data load_image(const char *name) {
    ...
    image_data img = {0};
    char *fixed_name = path_name("../res/pics/", name, ".png");
...
char *load_vertex_shader(const char *name) {
    char *fixed_name = path_name("../res/shaders/", name, "_v.glsl");
    FILE *file = plt_fopen(fixed_name, "rb");
    xfree(fixed_name);
    if (!file) {
...
char *load_fragment_shader(const char *name) {
    char *fixed_name = path_name("../res/shaders/", name, "_f.glsl");
    FILE *file = plt_fopen(fixed_name, "rb");
    xfree(fixed_name);
    if (!file) {
